  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "Dashboard_tab",
              box(title = "Selection", width = 12, solidHeader = TRUE, status = "success",
                  fluidRow(
                    column(6,
                           selectInput("selectIrisColumn1", 
                                       label = h3("Select box"), 
                    ...
                    column(6,
                           selectInput("selectIrisColumn2", 
                                       ...
                                       choices = list("Sepal Length" = "Sepal.Length", 
                                                      ...
                                                      "Species" = "Species"), 
                                       selected = 1)),
                    box(title = "Sepals", width = 6, status = "success",
                        column(12,
                               plotOutput("testDataPlot1"))),
                    box(title = "Petals", width = 6, status = "success",
                        column(12,
...
server <- function(input, output) {
  ...
  output$testDataPlot1 <- renderPlot({
    ggplot(appData) + geom_point(aes_string("Sepal.Length", input$selectIrisColumn1))
  })
  
  output$testDataPlot2 <- renderPlot({
    ggplot(appData) + geom_point(aes_string("Petal.Length", input$selectIrisColumn2))
  })
